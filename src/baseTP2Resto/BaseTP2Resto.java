...
public class BaseTP2Resto {
    ...
    public static void main(String[] args) {
        ...
        // On crée un StringBuilder pour stocker tous les menus
        StringBuilder allMenusText = new StringBuilder();
        ...
        for (int i = 1; i <= number_of_menus; i++) {
            ...
            // Ajout du menu au StringBuilder pour écrire dans le fichier
            allMenusText.append("Menu n°").append(i).append(" : ").append(menu).append("\n");
        }

        scanner.close();

        // Écriture dans le fichier menus.txt
        System.out.println("Chemin complet du fichier : " + new java.io.File("menus.txt").getAbsolutePath());
        try (FileOutputStream fos = new FileOutputStream("menus.txt")) {
            fos.write(allMenusText.toString().getBytes());
            System.out.println("Tous les menus ont été enregistrés dans menus.txt");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
